...
def process_video_accident(request):
    if request.method == 'POST' and request.FILES.get('frame'):
        ...
        # If accident detected, send alert with image
        ...
        # Return detections and image in response for debugging (optional)
        _, buffer = cv2.imencode('.jpg', img)
        img_base64 = base64.b64encode(buffer).decode('utf-8')
        response = {'detections': detections, 'image': img_base64}
        return JsonResponse(response, safe=False)
